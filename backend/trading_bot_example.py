...
Bot qui gere qu'un seul wallet 
"""
...
class KNOTradingBot:
    ...
    def sell_kno(self, current_price):
        ...
        try:
            ...
            # Balance KNO
            self.rpc_sleep()
            balance_kno = self.from_wei(
                token_kno.functions.balanceOf(self.wallet_address).call(), 18
            )

            if balance_kno < min_swap_amount:
                self.logger.warning("Balance KNO insuffisante")
                return False
            ...
            receipt = self.wait_receipt_slow(tx_hash)
            if not receipt or receipt.status != 1:
                self.logger.error("Vente échouée")
                return False

            # Balance WPOL après
            self.rpc_sleep()
            gained_wei = token_wpol.functions.balanceOf(
                self.wallet_address
            ).call()
