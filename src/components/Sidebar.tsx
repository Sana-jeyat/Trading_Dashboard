...
function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  const { bots, selectedBot, selectedBotId, toggleBot, selectBot } = useBotContext();
  ...
  return (
    <div className="w-64 bg-white shadow-lg border-r border-gray-200 flex flex-col">
      <div className="p-6 border-b border-gray-200">
        ...
        {/* Bot Selector */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Bot actuel</label>
          <select
            value={selectedBotId}
      ...
      <div className="p-4 border-t border-gray-200">
        <div className="text-center">
          <div className={`w-3 h-3 rounded-full mx-auto mb-2 ${
            selectedBot.isActive ? 'bg-green-400 animate-pulse' : 'bg-gray-400'
          }`}></div>
          <p className="text-sm text-gray-600">
            {selectedBot.isActive ? 'Bot en ligne' : 'Bot hors ligne'}
          </p>
          <p className="text-xs text-gray-500 mt-1">{selectedBot.tokenPair}</p>
        </div>
      </div>
    </div>
  );
